...
{
  expect_true(.assign.settings(connections))
})
...
test_that('.assignSettings',
{
  ...
  print(expression)
  list.file.servers <- ds.aggregate(expression = expression, datasources = connections)
  print("*********")
  print(list.file.servers)
})
...
test_that('.encrypt.data',
          {
            expect_true(TRUE)
            print(.assign.settings(connections))
            print(.encrypt.data(connections))
            expression <- call("lsDS", NULL, ".GlobalEnv")
            list.file.servers <- ds.aggregate(expression = expression, datasources = connections)
            print("*********")
            print(list.file.servers)
